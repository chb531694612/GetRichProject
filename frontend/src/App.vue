    ...
    <section class="content-frame surface">
      ...
      <div v-else class="plan-scroll">
        <article v-for="plan in result.items" :key="plan.plan_id" class="plan-card">
          ...
          <div class="legs-wrap">
            <table><thead><tr><th>场次 / 联赛</th><th>比赛</th><th>推荐</th><th>AI分析</th><th>SP</th><th>赛果</th><th>{{ plan.status === 'pending' && !plan.purchased ? '调整' : '结算' }}</th></tr></thead>
              <tbody><tr v-for="leg in plan.legs" :key="leg.match_id">
                ...
                <td class="settle-cell">
                  ...
                  <div class="settle-actions">
                    <button v-if="leg.ai_suggestion && leg.result.status === 'pending' && !plan.purchased" class="soft" @click="applyAiSuggestion(plan, leg)">替换为AI推荐</button>
