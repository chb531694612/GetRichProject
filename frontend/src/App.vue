...
<template>
  ...
  <div v-if="calendarOpen" class="modal-layer" @click.self="calendarOpen = false">
    <section class="calendar-modal surface">
      ...
      <div class="calendar-grid"><button v-for="(cell,index) in calendarCells" :key="index" :disabled="!cell" :class="{selected:cell?.date===filters.date,active:cell && calendarStats[cell.date]}" @click="cell && selectCalendarDate(cell.date)"><template v-if="cell"><b>{{ cell.day }}</b><span v-if="calendarStats[cell.date]">{{ calendarStats[cell.date].total }}张计划</span><small v-if="calendarStats[cell.date]?.pending">待结算{{ calendarStats[cell.date].pending }}</small><small v-else-if="calendarStats[cell.date]?.won" class="profit">中奖{{ calendarStats[cell.date].won }}</small><small v-else>—</small></template></button></div>
    </section>
  </div>
