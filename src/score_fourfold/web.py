...
.purchase-badge{background:#ecfdf3;color:#067647}.plan-actions .ticket-thumb{max-width:200px;max-height:150px;border-radius:8px;border:1px solid var(--line);margin-top:8px;cursor:pointer}.ticket-upload-form{display:inline-flex;align-items:center;margin-top:4px}.ticket-file-input{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}.ticket-drop-zone{width:130px;min-height:92px;border:1.5px dashed #c7cdd6;border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;text-align:center;color:var(--muted);background:#fcfcfd;cursor:pointer;padding:9px;touch-action:manipulation}.ticket-drop-zone:hover,.ticket-drop-zone.dragover{border-color:var(--blue);background:#eff6ff;color:var(--blue)}.ticket-upload-icon{font-size:28px;line-height:1;color:#12b8a6}.ticket-upload-count{font-size:11px}.screenshot-btn{background:#7c3aed!important;color:#fff!important;border-color:#7c3aed!important}.settle-btn{background:#475467;color:#fff;border-color:#475467}
...
@media(max-width:820px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}.top,.action{display:block}.action form{margin-top:14px}.plan-head{display:block}.badges{justify-content:flex-start;margin-top:8px}.calendar-day{min-height:60px}.plan-actions{align-items:flex-start}.plan-actions button,.plan-actions .btn-sm{min-height:40px}.ticket-drop-zone{width:150px;min-height:105px}table{min-width:980px}}
@media(max-width:480px){.grid{grid-template-columns:1fr}.wrap{padding:18px 12px 40px}.metric .value{font-size:24px}.calendar-day{min-height:50px;font-size:11px}.plan-money{gap:14px}.plan-money>div{min-width:42%}.plan-actions{padding:12px!important}.ticket-upload-form,.ticket-drop-zone{width:100%}.ticket-drop-zone{box-sizing:border-box}.toast-container{left:12px;right:12px;max-width:none}}
"""
...
function uploadTicketFile(form){var fd=new FormData(form);fd.set('csrf_token',__TOKEN__);var zone=form.querySelector('.ticket-drop-zone');if(zone){zone.setAttribute('aria-busy','true');zone.querySelector('.ticket-upload-text').textContent='上传中…'}return fetch('/actions/upload-ticket',{method:'POST',headers:{'X-Requested-With':'XMLHttpRequest'},body:fd}).then(function(r){return r.json()}).then(function(result){showToast(result.level||'warn',result.detail||'上传完成');if(result.plan_html&&result.plan_id){refreshPlanCard(result.plan_id,result.plan_html)}return result}).catch(function(err){showToast('error','上传失败：'+err.message)}).finally(function(){if(zone){zone.removeAttribute('aria-busy');zone.querySelector('.ticket-upload-text').textContent='点击或拖拽上传'}})}
function canvasBlob(canvas){return new Promise(function(resolve){canvas.toBlob(resolve,'image/png')})}
function wrapCanvasText(ctx,text,x,y,maxWidth,lineHeight){var words=Array.from(text||'');var line='';var lines=[];words.forEach(function(ch){var test=line+ch;if(ctx.measureText(test).width>maxWidth&&line){lines.push(line);line=ch}else line=test});if(line)lines.push(line);lines.forEach(function(item,index){ctx.fillText(item,x,y+index*lineHeight)});return y+lines.length*lineHeight}
async function screenshotPlan(target){var plan=target.closest('.plan');if(!plan)return;var title=plan.querySelector('.plan-title');var rows=Array.from(plan.querySelectorAll('tbody tr'));var width=1080,pad=52,rowHeight=92,height=210+rows.length*rowHeight;var canvas=document.createElement('canvas');var scale=Math.min(window.devicePixelRatio||1,2);canvas.width=width*scale;canvas.height=height*scale;var ctx=canvas.getContext('2d');ctx.scale(scale,scale);ctx.fillStyle='#fff';ctx.fillRect(0,0,width,height);ctx.fillStyle='#101828';ctx.font='bold 30px sans-serif';ctx.fillText(title?title.textContent.trim():'竞彩足球推荐',pad,58);ctx.font='16px sans-serif';ctx.fillStyle='#667085';ctx.fillText('生成时间：'+new Date().toLocaleString('zh-CN',{hour12:false}),pad,90);ctx.strokeStyle='#d0d5dd';ctx.beginPath();ctx.moveTo(pad,112);ctx.lineTo(width-pad,112);ctx.stroke();ctx.fillStyle='#344054';ctx.font='bold 17px sans-serif';ctx.fillText('比赛',pad,145);ctx.fillText('对阵',245,145);ctx.fillText('推荐赛果',760,145);var y=180;rows.forEach(function(row){var cells=row.querySelectorAll('td');var match=cells[0]?cells[0].innerText.trim().replace(/\n/g,' / '):'';var teams=cells[2]?cells[2].innerText.trim():'';var pick=cells[3]?cells[3].querySelector('strong'):null;ctx.fillStyle='#101828';ctx.font='16px sans-serif';wrapCanvasText(ctx,match,pad,y,170,23);wrapCanvasText(ctx,teams,245,y,460,23);ctx.fillStyle='#7c3aed';ctx.font='bold 22px sans-serif';wrapCanvasText(ctx,pick?pick.textContent.trim():'',760,y,250,27);ctx.strokeStyle='#eaecf0';ctx.beginPath();ctx.moveTo(pad,y+55);ctx.lineTo(width-pad,y+55);ctx.stroke();y+=rowHeight});ctx.fillStyle='#667085';ctx.font='14px sans-serif';ctx.fillText('仅供到店出票核对，请以实体票内容为准',pad,height-28);var blob=await canvasBlob(canvas);if(!blob)throw new Error('图片生成失败');try{if(!navigator.clipboard||!window.ClipboardItem)throw new Error('clipboard unavailable');await navigator.clipboard.write([new ClipboardItem({'image/png':blob})]);showToast('ok','推荐图片已复制到剪贴板，可直接粘贴发送')}catch(err){var link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download=(target.dataset.planId||'推荐结果')+'.png';link.click();setTimeout(function(){URL.revokeObjectURL(link.href)},1000);showToast('warn','浏览器不允许复制图片，已自动保存 PNG 文件')}}
...
document.addEventListener('click',function(event){var target=event.target.closest('[data-action]');if(!target)return;event.preventDefault();var action=target.dataset.action;if(action==='delete-plan')delPlan(target.dataset.planId);else if(action==='delete-leg')delLeg(target.dataset.planId,target.dataset.matchId);else if(action==='analyze-plan')runAIAnalysis(target);else if(action==='screenshot-plan')screenshotPlan(target).catch(function(err){showToast('error','截图失败：'+err.message)});else if(action==='replace-ai')updateLeg(target,target.dataset.optionCode);else if(action==='save-leg'){var select=document.getElementById(target.dataset.selectId);if(select){target.dataset.optionLabel=select.options[select.selectedIndex].text;updateLeg(target,select.value)}}else if(action==='mark-purchased')markPurchased(target.dataset.planId,target.dataset.purchased==='1');else if(action==='settle')triggerSettle();else if(action==='settle-plan')settlePlan(target.dataset.planId);else if(action==='open-modal')openModal(target.dataset.modalId);else if(action==='close-modal')closeModal(target.dataset.modalId)})
document.addEventListener('change',function(event){if(event.target.matches('.ticket-file-input')&&event.target.files&&event.target.files.length){uploadTicketFile(event.target.form)}})
document.addEventListener('dragover',function(event){var zone=event.target.closest('.ticket-drop-zone');if(zone){event.preventDefault();zone.classList.add('dragover')}})
document.addEventListener('dragleave',function(event){var zone=event.target.closest('.ticket-drop-zone');if(zone)zone.classList.remove('dragover')})
document.addEventListener('drop',function(event){var zone=event.target.closest('.ticket-drop-zone');if(!zone)return;event.preventDefault();zone.classList.remove('dragover');var file=event.dataTransfer&&event.dataTransfer.files&&event.dataTransfer.files[0];if(!file)return;if(!file.type.startsWith('image/')){showToast('warn','请选择图片文件');return}var input=zone.closest('form').querySelector('.ticket-file-input');var transfer=new DataTransfer();transfer.items.add(file);input.files=transfer.files;uploadTicketFile(input.form)})
    ...
    def _render_plan(self, plan: StoredPlan, csrf_token: str = "") -> str:
        ...
        if csrf_token and plan.delivery_status == "sent":
            ...
            purchase_btns += (
                ...
                f'<input class="ticket-file-input" id="ticket-{_e(pid)}" type="file" name="ticket_image" '
                'accept="image/*" required>'
                f'<label class="ticket-drop-zone" for="ticket-{_e(pid)}" tabindex="0">'
                '<span class="ticket-upload-icon" aria-hidden="true">☁↑</span>'
                '<span class="ticket-upload-text">点击或拖拽上传</span>'
                '<span class="ticket-upload-count">支持手机相册与拍照</span></label></form>'
            )
...
<div class="plan-actions" style="padding:8px 20px">{ai_btn} {settle_plan_btn} {del_btn}</div></article>"""
        ...
        screenshot_btn = (
            f'<button type="button" data-action="screenshot-plan" data-plan-id="{_e(pid)}" '
            'class="btn-sm screenshot-btn">一键截图推荐</button>' if csrf_token else ""
        )
...
<div class="plan-actions" style="padding:8px 20px">{screenshot_btn} {ai_btn} {settle_plan_btn} {del_btn} {purchase_btns}</div>{ai_modal}</article>"""
