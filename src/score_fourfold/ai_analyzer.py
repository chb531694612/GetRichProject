...
def _build_prompt(matches: list[tuple[Any, ScoreOption]], market: MarketType) -> str:
    lines: list[str] = [
        ...
        "5. 分析要有胆量：只要搜索证据支持，就要敢于给出非主流判断（如进球数看 5 球、6 球甚至 7+，或直接点名弱队拿分/爆冷），不要因为结果少见或赔率较高就回避，更不要只顺着纸面实力和热门方向分析。",
        "",
        "系统只提供以下基础赛程信息：",
    ]
...
def _qwen_response(prompt: str, settings: Settings, *, max_tokens: int) -> str:
    ...
    payload = {
        ...
        "input": [
            {
                "role": "system",
                "content": "你是一名严谨的足球比赛信息分析师。必须先联网检索近期公开资料，综合战绩、交锋、伤停、赛程和社区观点进行交叉验证。足球比赛充满变数，你必须主动评估爆冷风险，不得默认选择强队或热门方向。分析要有胆量：只要证据支持，就敢于给出高进球数、大比分、弱队拿分等非主流判断，不要回避冷门。严禁敷衍了事或给出空泛结论。严格按用户要求输出。",
            },
...
def _build_plan_recommendation_prompt(legs: Sequence[Any], market: MarketType) -> str:
    ...
    lines.extend(
        ...
        "",
        "大胆性要求（同样适用于每场比赛）：",
        "1. 进球数（TTG）玩法：只要证据支持，就要敢于选择 5、6 甚至 7+ 这类高进球数，不要习惯性只选小数字；当两队进攻火力强、防守薄弱、交锋进球多或伤停导致防线崩坏时，高进球数往往是更合理的选择。",
        "2. 比分（CRS）玩法：不要永远只选 1:0、1:1、2:1 这类常见比分；若数据显示大比分可能（如两队近期进球多、防守差），应大胆给出 3:2、4:2 甚至更高比分的判断。",
        "3. 高赔率、冷门选项并不可怕：只要搜索到的证据链清晰，推荐看似'不像会中'的结果是允许的，也是你的职责所在；不要为了显得稳妥而放弃有证据支撑的冷门判断。",
        "4. 大胆不是瞎猜：任何大胆选择都必须有具体搜索依据支撑，并写进 reason；没有证据支持时不得随意编造冷门。",
        "",
